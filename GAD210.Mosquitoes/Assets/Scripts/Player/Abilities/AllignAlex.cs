...
public class AlignAlex : MonoBehaviour
{
    [Header("Settings")]
    ...
    {
        if (Physics.Raycast(transform.position, transform.forward, out RaycastHit hit, rayDistance, surfaceMask))
        {

            targetSurface = hit.collider.transform;

            var setter = targetSurface.GetComponent<MosquitePositionSetter>();
            if (setter && setter.obj != null)
            {

                targetSurface = setter.obj;
    ...
    private void AlignInLocal()
    {
        ...
        bool rotDone = Quaternion.Angle(transform.localRotation, targetLocalRot) < 0.5f;
